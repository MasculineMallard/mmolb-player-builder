...
- **Scheduler → instrumentation**: moved the import-time `setInterval`/`setTimeout` block out of `percentile-builder.ts` into an exported `startPercentileScheduler()` (idempotent via a `schedulerStarted` guard, production-gated as before). New `src/instrumentation.ts` `register()` calls it on the `nodejs` runtime only.
  - **VERIFIED on the pr-54 Railway preview** (logs): the hook fires at boot and the startup crawl completes — `[percentiles] Cache stale or missing, auto-refreshing on startup... Found 3162 teams... Refresh complete. 7314 batters, 7820 pitchers. Saved to disk.`
  - **REGRESSION found + fixed:** despite the crawl saving, `/api/health` reported `percentiles:"none"`. Next bundles `instrumentation.ts` into a **different module instance** than the API routes, so the scheduler populated *its* in-memory `cached` while the routes' instance kept `null` (and `loadFromDisk()` only runs once at import, before the crawl saved). The old import-time start didn't hit this because the scheduler ran inside the route's own instance. **Fix:** `getCachedPercentiles` now syncs from the on-disk cache (TTL-guarded, 60s; adopts disk if newer than in-memory). The disk file is the cross-instance source of truth — also hardens the multi-worker scale-out case SCRATCH flagged. Re-verify on the redeployed preview that `/pop/api/health` flips to `percentiles:"fresh"`.
